...
本报告针对"主路由器 + 副路由器"双路由网络架构进行技术可行性评估。该方案的核心需求为：主路由器承载家庭日常网络，副路由器通过 WireGuard 隧道 24 小时连接公司网络，用户通过切换 WiFi 即可切换网络出口，同时工作网络需保持对 NAS 文件访问及 AirPlay/HomeKit 设备发现的能力。经分析，该方案在技术层面整体可行，但 mDNS 跨子网中继在 HomeKit 场景下存在已知可靠性问题，需通过配置优化和备选策略缓解。
...
- 预算范围：副路由器 150-350 元
- 智能家居品牌：绿米 (Aqara)、欧瑞博 (ORVIBO)，均在主网配对
- 主要控制路径：Agent → Home Assistant API → 智能家居（TCP，跨子网可靠）
...
| 协议/服务 | 跨子网可靠性 | 说明 |
|-----------|-------------|------|
| **Home Assistant API** | ✅ 完全可靠 | 基于 HTTP/WebSocket (TCP)，NAT 穿透后正常工作，不依赖 mDNS |
| **Agent 系统** | ✅ 完全可靠 | 通过 Home Assistant API 交互，TCP 单播，完全可靠 |
...
**实际影响评估**：由于用户的主要控制路径为 Agent 系统 → Home Assistant API → 智能家居设备（绿米/欧瑞博），该路径基于 TCP 协议，跨子网完全可靠。HomeKit 的 mDNS 限制仅影响通过 Siri/家庭 App 直接控制的场景，不影响主要控制路径。

#### 缓解策略

1. **优先使用 Agent / Home Assistant 控制**：主要控制路径基于 TCP，跨子网完全可靠，不受 mDNS 限制影响
2. **HomeKit 设备配对**：所有设备（绿米/欧瑞博）在主网络配对完成，当前已是如此
3. **avahi 服务过滤**：仅转发必要的服务类型（`_airplay._tcp`, `_hap._tcp` 等），减少 mDNS 噪声
4. **reflector-quick-join**：启用快速加入模式，缩短发现延迟
5. **防火墙放行**：确保 UDP 5353 端口在两个子网间畅通
...
| **CMCC RAX3000M** | MT7981B | 512MB/128MB | WiFi 6 AX3000 | 4× 千兆 + USB 3.0 | 官方支持 | ¥150-250 (二手) |
...
| 风险 | 概率 | 影响 | 缓解措施 |
|------|------|------|---------|
| HomeKit 设备间歇性"无响应" | 高 | 低 | 主要控制路径为 Agent/HA (TCP)，HomeKit 仅辅助；首次配对在主网完成 |
...
1. **HomeKit 跨子网非完美体验**：这是 mDNS 中继的架构性限制，无法通过配置完全消除。但由于主要控制路径为 Agent/HA，实际影响有限
...
| Agent/HA 控制 | ✅ TCP 完全可靠 | ✅ TCP 完全可靠 | ✅ TCP 完全可靠 | ✅ TCP 完全可靠 |
...
**方案 A（双子网 + mDNS 中继）技术可行，HomeKit 风险对实际使用影响有限。** 核心评估如下：

- WireGuard 隧道性能：✅ 充裕（355-470 Mbps，远超办公需求）
- NAS 文件访问：✅ 完全可靠（NAT 穿透无障碍）
- Agent/HA 智能家居控制：✅ 完全可靠（TCP 协议，不受 mDNS 影响）
- AirPlay 投屏：⚠️ 基本可用（发现延迟 5-15 秒，连接后稳定）
- HomeKit 控制：⚠️ 有风险（偶发"无响应"，但仅影响辅助控制路径）
- 主路由零改动：✅ 完全可行
